...
def SDXL_reconstruction(eegmodel, dataloader, DM_prior,SDXL,device,image_size=(256, 256),subject_id=None,guidance_scale=50.0):
    ...
    with torch.no_grad():
        for batch_idx, (img, eeg_data) in enumerate(dataloader): 
            ...
            h = DM_prior.generate(c_embeds=eeg_features, num_inference_steps=50, guidance_scale=guidance_scale)
            eeg_gen = SDXL.generate(h.to(dtype=torch.float16))
...
def main():
    ...
    image_list,recon_list = SDXL_reconstruction(eeg_model, 
                                                test_loader,DM_prior=pipe,SDXL=generator,
    ...
    recon_dir = os.path.join(config['model_path'], f"DMHighlevel_reconstructions/{config['model_type']}_{config['subject_id'][0]}")
    ...
    csv_file_path =os.path.join(config['model_path'], "DMHighlevel_model_results.csv")
