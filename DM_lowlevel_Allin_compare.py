...
def main():
    ...
    refined_eeg = DM_eeg_features(eeg_featurelist, lowlevel_DM, device, guidance_scale=50.0, batch_size=30)
    ...
    recon_dir = os.path.join(config['model_path'], f"LowlevelAllin_reconstructions/DM_{config['model_type']}_{config['subject_id'][0]}_avg{config['average_eeg']}")
    ...
    csv_file_path =os.path.join(config['model_path'], "DMlowlevel_Allin_results.csv")
