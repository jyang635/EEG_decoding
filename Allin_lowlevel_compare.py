...
from ALLin_lowlevel import ATMS_DM
...
def geteeg_features(eegmodel, dataloader,device,image_size=(256, 256),model_type='ATMS_Deconv',subject_id=None):
    ...
    with torch.no_grad():
        for batch_idx, (img, eeg_data) in enumerate(dataloader):            
            ...
            eeg_features = eegmodel.generate(eeg_data,subject_ids,num_inference_steps=50, guidance_scale=50)
...
def main():
    ...
    checkpoint = torch.load(f"{config['model_path']}/Lowlevel_Allin/{config['model_type']}/{config['subject_id'][0]}/lowlevel_DM.pth", map_location=device)
    ...
    vae.eval()
    print(f"VAE loaded")
    recon_list = VAE_reconstruction(eeg_featurelist, vae, device, batch_size=5, image_size=(256,256))
    ...
    recon_dir = os.path.join(config['model_path'], f"ALLin_Lowlevel_reconstructions/DM_{config['model_type']}_{config['subject_id'][0]}_avg{config['average_eeg']}")
    ...
    csv_file_path =os.path.join(config['model_path'], "ALLin_lowlevel_model_results.csv")
