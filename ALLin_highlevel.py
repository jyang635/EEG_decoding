...
from EEG_low_level_encoders import EEGConformer_Deconv,encoder_low_level,encoder_low_level_channelwise,ATMS,Config
...
class ATMS_DM(nn.Module):
    def __init__(self, config):
        ...
        self.eeg_model = ATMS(config)
        self.diffusion_model = DiffusionPriorUNet(cond_dim=1024,dropout=0.1)
    ...
    def generate(self, x, subject_ids,scheduler=None,num_inference_steps=50, timesteps=None,guidance_scale=5.0, generator=None,device='cuda'):
        # Pass EEG data through the EEG model
        if x is not None:
            eeg_features = self.eeg_model(x, subject_ids)
        else:
        ...
        N = eeg_features.shape[0]if eeg_features is not None else 1
        ...
        h_t = torch.randn(N, self.diffusion_model.embed_dim, generator=generator, device=device)

        # 4. denoising loop
        for _, t in tqdm(enumerate(timesteps)):
            t = torch.ones(h_t.shape[0], dtype=torch.float, device=device) * t
            # 4.1 noise prediction
            if guidance_scale == 0 or eeg_features is None:
                noise_pred = self.diffusion_model(h_t, t)
            else:
                noise_pred_cond = self.diffusion_model(h_t, t, eeg_features)
                noise_pred_uncond = self.diffusion_model(h_t, t)
...
def main_train_loop(eeg_model, train_dataloader, validation_dataloader,device, 
    ...
    for t in range(config['epochs']):
        ...
        if stop_counter >= config['early_stopping_patience']:
            ...
            #     model_path=f"./models/Highlevel_Allin/{config['model_type']}/{subject_ids[0]}"
    ...
    if config['save_model']:
        # Save the model
        model_path=f"./models/Highlevel_Allin/{config['model_type']}/{config['subject_id'][0]}"
        print(f"Saving model to {model_path}")
        os.makedirs(model_path, exist_ok=True)             
        file_path = f"{model_path}/Highlevel_DM.pth"
        torch.save(best_model_state, file_path)
