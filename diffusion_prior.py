...
class lowlevel_DiffusionPriorUNet(nn.Module):
    ...
    def __init__(
        ...
        self.encode_time_embedding = nn.ModuleList(
            [TimestepEmbedding(time_embed_dim, hidden_channels[i]) for i in range(self.num_layers-1)]
        )
        self.encode_cond_embedding = nn.ModuleList(
            [nn.Linear(cond_dim, hidden_channels[i]) for i in range(self.num_layers-1)]
        )
        
        self.encode_blocks = nn.ModuleList([])
        self.encode_downsamples = nn.ModuleList([])
        
        for i in range(self.num_layers-1):
            self.encode_blocks.append(nn.Sequential(
                nn.Conv2d(hidden_channels[i], hidden_channels[i], kernel_size=3, padding=1),
            ...
            self.encode_downsamples.append(nn.Sequential(
        ...
        self.decode_time_embedding = nn.ModuleList(
            [TimestepEmbedding(time_embed_dim, hidden_channels[i]) for i in range(self.num_layers-1, 0, -1)]
        )
        self.decode_cond_embedding = nn.ModuleList(
            [nn.Linear(cond_dim, hidden_channels[i]) for i in range(self.num_layers-1, 0, -1)]
        )
        
        self.decode_upsamples = nn.ModuleList([])
        self.decode_blocks = nn.ModuleList([])
        
        for i in range(self.num_layers-1, 0, -1):
            self.decode_upsamples.append(nn.Sequential(
            ...
            self.decode_blocks.append(nn.Sequential(
                nn.Conv2d(hidden_channels[i-1] * 2, hidden_channels[i-1], kernel_size=3, padding=1), # *2 for skip connection
                nn.BatchNorm2d(hidden_channels[i-1]),
                act_fn(),
                nn.Dropout(dropout),
                nn.Conv2d(hidden_channels[i-1], hidden_channels[i-1], kernel_size=3, padding=1),
                nn.BatchNorm2d(hidden_channels[i-1]),
                act_fn(),
            ))
        
        # 6. output
        self.output_conv = nn.Conv2d(hidden_channels[0], in_channels, kernel_size=3, padding=1)
        
    def _apply_time_cond_embedding(self, x, t_emb, c_emb=None):
        # Reshape embeddings to match spatial dimensions for broadcast addition
    ...
    def forward(self, x, t, c=None):
        ...
        # t: (batch_size, )
        # c: (batch_size, cond_dim) or None
        
        # 1. time embedding
        t = self.time_proj(t)  # (batch_size, time_embed_dim)
        
        # 2. initial convolution
        x = self.init_conv(x)  # (batch_size, hidden_channels[0], 64, 64)
        
        # 3. encoder path with skip connections
        skip_connections = []
        for i in range(self.num_layers-1):
            # Apply block
            ...
            # Apply time and condition embeddings
            t_emb = self.encode_time_embedding[i](t)
            c_emb = self.encode_cond_embedding[i](c) if c is not None else None
            x = self._apply_time_cond_embedding(x, t_emb, c_emb)
            
            # Downsample
            x = self.encode_downsamples[i](x)
        ...
        for i in range(self.num_layers-1):
            # Apply time and condition embeddings
            t_emb = self.decode_time_embedding[i](t)
            c_emb = self.decode_cond_embedding[i](c) if c is not None else None
            x = self._apply_time_cond_embedding(x, t_emb, c_emb)
            
            # Upsample
            x = self.decode_upsamples[i](x)
