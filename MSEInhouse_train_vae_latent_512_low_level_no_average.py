...
import tqdm
from Inhouse_eegdatasets_leaveone_latent_vae_no_average import EEGDataset
...
import itertools
import csv
from JY_low_level_encoders import Deconv_EEGConformer
...
conformer_model=Deconv_EEGConformer(n_outputs=2, n_chans=63, n_filters_time=180, 
                             filter_time_length=20, pool_time_length=5, pool_time_stride=5, 
                             drop_prob=0.5, att_depth=3, att_heads=30, att_drop_prob=0.5, 
                             final_fc_length='auto', return_features=False, n_times=250, 
                             chs_info=None, input_window_seconds=None, sfreq=None, n_classes=None, 
                             n_channels=None, input_window_samples=None, add_log_softmax=True)
...
def train_model(eegmodel, imgmodel, dataloader, optimizer, device, text_features_all, img_features_all, save_dir, epoch):
    ...
    for batch_idx, (eeg_data, labels, _, img_features) in enumerate(dataloader):
        ...
        #             for k in range(0, len(eeg_features), 10):
        #                 z= eeg_features[k:k+10].to('cuda:1')
        #                 z_i=img_features[k:k+10].to('cuda:1')
        #                 # z = eeg_features.to('cuda:1')
        #                 x_rec = vae.decode(z).sample
        #                 x_train = vae.decode(z_i).sample
        ...
        #                 current_labels= labels[k:k+10].tolist()
...
def evaluate_model(eegmodel, imgmodel, dataloader, device, text_features_all, img_features_all, k, save_dir, epoch):
    ...
    # print("img_features_all", img_features_all.shape)
    total_loss = 0
    mse_loss_fn = nn.MSELoss()
    mae_loss_fn = nn.L1Loss()
    ridge_lambda = 0.1
...
def main_train_loop(sub, current_time, eeg_model, img_model, train_dataloader, test_dataloader, optimizer, device, 
    ...
    for epoch in range(config.epochs):
        ...
        # Add date-time prefix to save_dir
        test_save_dir = f'{current_time}_vae_test_imgs'
        ...
        if test_loss < best_test_loss:
            ...
            best_model_weights = copy.deepcopy(eeg_model.state_dict())
            best_epoch=epoch
            testloss_monitor = 0
        else:
            testloss_monitor += 1
            if testloss_monitor > 10:
    ...
    if config.insubject==True:
        os.makedirs(f"./models/contrast/{config.encoder_type}/{sub}/{current_time}", exist_ok=True)             
        file_path = f"./models/contrast/{config.encoder_type}/{sub}/{current_time}/best_model_epoch{best_epoch}.pth"
        torch.save(best_model_weights, file_path)
        file_path = f"./models/contrast/{config.encoder_type}/{sub}/{current_time}/model_{epoch}.pth"
        torch.save(eeg_model.state_dict(), file_path)
    else:
        os.makedirs(f"./models/contrast/across/{config.encoder_type}/{current_time}", exist_ok=True)             
        file_path = f"./models/contrast/across/{config.encoder_type}/{current_time}/besbest_model_epoch{best_epoch}t_model.pth"
        torch.save(best_model_weights, file_path)
        file_path = f"./models/contrast/{config.encoder_type}/{sub}/{current_time}/model_{epoch}.pth"
        torch.save(eeg_model.state_dict(), file_path)
    print(f"model saved in {file_path}!")
...
def main():
    ...
    parser.add_argument('--encoder_type', type=str, default='encoder_low_level', 
                        choices=['conformer_model', 'encoder_low_level'], 
                        help='Encoder type')
    ...
    for sub in subjects:
        ...
        if args.encoder_type == 'conformer_model':
        ...
        # print('shape of text features'+ str(text_features_train_all.shape))
